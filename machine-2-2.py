...
# 훈련 시작 (에포크 5000번으로 설정하여 충분히 학습되도록 함)
# Iris 데이터는 스케일이 다르므로 에포크를 높였습니다.
neuron.fit(x, y, epochs=5000)

# 4. 학습 결과 시각화
plt.figure(figsize=(8, 6))
plt.scatter(x, y, label='Iris Data (Petal Length vs Sepal Length)', alpha=0.6)
...
# 학습된 직선 그리기
plt.plot([pt1[0], pt2[0]], [pt1[1], pt2[1]], 'r-', label='Learned Regression Line')

plt.xlabel('X: 꽃잎 길이 (Petal Length)', fontsize=12)
plt.ylabel('Y: 꽃받침 길이 (Sepal Length)', fontsize=12)
plt.title('Iris 데이터 선형 회귀 (Neuron 구현)', fontsize=14)
plt.legend()
...
plt.scatter(x_new, y_pred, color='red', marker='X', s=200, label=f'Predicted Point ({y_pred:.2f})')
